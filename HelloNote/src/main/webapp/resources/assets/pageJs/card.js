...
// click listener for clip copy
//클립카드 가져오기
$(document).on("click", ".copy-clip", function (e) {

  var sel_cc_active = $("[class*=cc-active]")[0];

  console.log(sel_cc_active);
  var ccnumval = $("[class*=cc-active]")[0].getElementsByClassName("cc-num")[0].textContent;
  var ccdate = $("[class*=cc-active]")[0].getElementsByClassName("cc-date")[0].textContent;
  var ccid = $("[class*=cc-active]").attr('id');

  var cliptext = ccnumval + " , " + ccdate + " , " + ccid;
  var tempElem =  document.createElement('textarea')
  tempElem.value = cliptext;
...
$(document).on("click",".list-remove",function(e){
   var sel_cc_active = $("[class*=cc-active]");
   sel_cc_active.remove();
});
...
//순서변경닫기
$(document).on("click", ".modal-list-close", function (e) {
  $(".modal-cardlist-change").hide();
  var chi =  $("#divOuter1").children();
  chi.remove();
});

//리스트 순서변경  modal 열기
$(".modal-list-open").on("click", function (e) {
  var clo = $(".cc-select").clone().children();
  // console.log(clo);
  // console.log($("#divOuter1"));
  $("#divOuter1").append(clo);
  $(".modal-cardlist-change").show();
});

//패스워드 입력 open
function modalCheckPassword(){
  var ccnumval = $("[class*=cc-active]")[0].getElementsByClassName("cc-num")[0].textContent;
  $("#innerCardNum")[0].value=ccnumval;
  $(".modal-passwordinput").show();
}
// 은행 거래내역 조회
$(document).on("click","#selectBankTransactionalInfo",function(e){
  if(true){
    load($(".cc-active").attr("id"), data);
  }
  $(".modal-passwordinput").hide();
  
});

//패스워드 입력 close
$(document).on("click", ".modal-passwordinput-close", function (e) {
  $(".modal-passwordinput").hide();
});
...
$(document).on("click", ".modal-add-cc", function (e) {
  ...
  if (ccNum && month && year && ccType) {
    ...
    ccModalAppend(ccType, ccNum, month, year)// need check
